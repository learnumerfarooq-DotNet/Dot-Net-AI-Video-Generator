...
export class SettingsAgentComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder) {
    ...
    effect(() => {
      const agent = this.store.activeAgentSettings();
      if (agent) {
  ...
  save(): void {
    ...
    if (agent) {
      ...
      Object.keys(val).forEach((key) => {
        this.store.updateSettingsDraft(agent.agentKey, key as any, val[key]);
      });
      void this.store.saveAgentSettings(agent.agentKey);
    }
  }

  get providerOptions(): string[] {
    return this.store.providerOptionsForActiveAgent();
  }
